...
class EolesModel:
    ...
    def load_param(self, path, name_load_factor="vre_profiles2006.csv", name_gene_river="run_of_river.csv"):
        ...
        # VRE profiles per hours (offshore + onshore + PV)
        self.load_factor = pd.read_csv(path + name_load_factor, index_col=[0, 1], header=None).squeeze("columns")
        ...
        self.demand_2050 = pd.read_csv(path + "demand2050_RTE.csv", index_col=0, header=None).squeeze("columns")
        ...
        self.gene_river = pd.read_csv(path + name_gene_river, header=None, index_col=0).squeeze("columns")
        # max Q
        self.max_capa = pd.read_csv(path + "max_capas.csv", header=None, index_col=0).squeeze("columns")
    ...
    def init_variable(self):
        ...
        self.model.volume = pyo.Var(self.model.str, within=pyo.NonNegativeReals, initialize=0,
                                    bounds=self.capacity_bounds)
    ...
    def objective_rule(self, model):
        ...
        return (sum((model.Q[tec] - self.capa_ex[tec]) * self.capex[tec] for tec in self.model.tec) \
                ...
                + sum(
                    model.S[storage_tecs] * (self.s_opex[storage_tecs] + self.s_capex[storage_tecs]) for storage_tecs in
                    self.model.str) \
    ...
    def cost_by_tec(self, tec):
        ...
        if tec in self.stor:
            const1 += (self.model.volume[tec]) * self.capex_en[tec] + self.model.S[tec] * (
                        self.s_opex[tec] + self.s_capex[tec])
