...
class SimplifiedModel:
    def __init__(self, Q_tec, Volume_str, S):
        ...
        print("Technologies used :", [i for i in self.tec if self.Q_tec[i] > 0])
    ...
    def load_param(self, path):
        ...
        # VRE profiles per hour (offshore + onshore + PV)
        self.load_factor = pd.read_csv(path + "vre_profiles2006.csv", index_col=[0, 1], header=None).squeeze("columns")
        # Additional FRR requirement for variable renewable energies because of forecast errors
        self.epsilon = pd.read_csv(path + "reserve_requirements.csv", index_col=0, header=None).squeeze("columns")
        # Demand profile in each our in GW
        # self.demand_2050 = pd.read_csv(path + "demand2050_negawatt.csv",index_col=0, header=None).squeeze("columns")
        self.demand_2050 = pd.read_csv(path + "demand2050_ademe.csv", index_col=0, header=None).squeeze("columns")
        # self.demand_2050 = pd.read_csv(path + "demand2050_ademe_noise.csv",index_col = 0, header=None).squeeze("columns")[1:]
        # Variable operation and maintenance costs in M€/GWh
        self.vOM = pd.read_csv(path + "vO&M.csv", index_col=0, header=None).squeeze("columns")
        # Monthly lake inflows in GWh
        self.lake_inflows = pd.read_csv(path + "lake_inflows.csv", index_col=0, header=None).squeeze("columns")
        # Rivers profiles per hour
        self.gene_river = pd.read_csv(path + "run_of_river.csv", header=None, index_col=0).squeeze("columns")

        # Annualized power capex cost in M€/GW/year
        self.capex = pd.read_csv(path + "annuities.csv", index_col=0, header=None).squeeze("columns")
        # Existing capacities of the technologies by December 2017 in GW
        self.capa_ex = pd.read_csv(path + "existing_capas.csv", index_col=0, header=None).squeeze("columns")
        # Annualized energy capex cost of storage technologies in M€/GWh/year
        self.capex_en = pd.read_csv(path + "str_annuities.csv", index_col=0, header=None).squeeze("columns")
        # Annualized fixed operation and maintenance costs M€/GW/year
        self.fOM = pd.read_csv(path + "fO&M.csv", index_col=0, header=None).squeeze("columns")
        ...
        # Maxium energy that can be generated by biogas in TWh
        self.max_biogas = 15
        # Uncertainty coefficient for hourly demand
        self.load_uncertainty = 0.01
        # Load variation factor
        self.delta = 0.1
        # Charging eficiency of storage technologies
        ...
        self.fOM = self.fOM.rename(
            index={"Offshore": "offshore", "Onshore": "onshore", "PV": "pv", "PHS": "phs", "Battery": "battery"})
        self.capex = self.capex.rename(
            index={"Offshore": "offshore", "Onshore": "onshore", "PV": "pv", "PHS": "phs", "Battery": "battery"})

    def init_set(self):

        print("Sets initialization...")
        self.first_hour = 0
        self.last_hour = len(self.demand_2050)
        days_in_feb = 672
        hours_by_months = {1: 744, 2: days_in_feb, 3: 744, 4: 720, 5: 744, 6: 720, 7: 744, 8: 744, 9: 720, 10: 744,
                           11: 720, 12: 744}
    ...
    def init_variable(self):
        ...
        print("Definition of variables to optimize ...")
        # Hourly energy generation in GWh/h
        self.model.gene = pyo.Var(((tec, h) for tec in self.model.tec for h in self.model.h),
                                  within=pyo.NonNegativeReals, initialize=0)

        # Hourly electricity input of battery storage GW
        self.model.storage = pyo.Var(((storage, h) for storage in self.model.str for h in self.model.h),
                                     within=pyo.NonNegativeReals, initialize=0)

        # Energy stored in each storage technology in GWh = Stage of charge
        self.model.stored = pyo.Var(((storage, h) for storage in self.model.str for h in self.model.h),
                                    within=pyo.NonNegativeReals, initialize=10)

        # Required upward frequency restoration reserve in GW    
        self.model.reserve = pyo.Var(((reserve, h) for reserve in self.model.frr for h in self.model.h),
                                     within=pyo.NonNegativeReals, initialize=0)
    ...
    def add_constraints(self):
        ...
        self.model.generation_vre_constraint = pyo.Constraint(self.model.h, self.model.vre,
                                                              rule=self.generation_vre_constraint_rule)
        self.model.generation_capacity_constraint = pyo.Constraint(self.model.h, self.model.tec,
                                                                   rule=self.generation_capacity_constraint_rule)
        ...
        self.model.stored_capacity_constraint = pyo.Constraint(self.model.h, self.model.str,
                                                               rule=self.stored_capacity_constraint)
        self.model.stored_capacity_constraint2 = pyo.Constraint(self.model.h, self.model.str,
                                                                rule=self.stored_capacity_constraint2)
    ...
    def run(self):
        ...
        self.optimisation()
        print("Model simulation successfully completed !")
    ...
    def cost(self):
        ...
        const4 = sum(self.S[storage_tecs] * (self.s_opex[storage_tecs] + self.s_capex[storage_tecs]) for storage_tecs in
                     self.stor)
        ...
        res = pd.DataFrame([[c_tot, c_mwh_produced]],
                           columns=["COST (billion euros)", "Cost per MWh produced (euros/MWh)"])
        return res
